...
class PropertyTest < ActiveSupport::TestCase
	...
	context 'When creating a property, the system' do 
		...
		should 'keep a local copy of value until explicitly saved' do 
			...
			p = Property.new(:name => "Monkey", :property_type_definition => "StringProperty", :default_value => "laser")
			assert_equal $redis.keys("*").length, 0
		...
		should 'not persist to redis if saving of property fails' do 
			assert_equal $redis.keys("*").length, 0
			p = Property.new(:name => nil, :property_type_definition => "StringProperty", :default_value => "laser")
			p.save
			assert !p.persisted? 
			assert_equal $redis.keys("*").length, 0
		end
	...
	context 'When cloning properties, the system' do 
		setup do 
			@property = Property.create(:name => "Monkey", :property_type_definition => "StringProperty", :default_value => "laser")
		end
		...
		should 'support cloning the property' do 
			...
			assert_equal p2.property_type_definition, @property.property_type_definition
